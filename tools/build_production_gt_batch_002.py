"""Provision PRODUCTION_GT_BATCH_002 under frozen stronger C/E sampling.

SPEC002 procedure + QC gates unchanged. No reuse of Batch 001 / validation sources.
"""
...
ACQ = REPO / "experiments/phase6e/PRODUCTION_GT_BATCH_002_SOURCE_ACQUISITION.json"
FAILED_001 = REPO / "experiments/phase6e/PRODUCTION_GT_BATCH_001_FAILED_CANDIDATE.json"
AS2_002 = REPO / "experiments/phase6e/AFFINITY_SPEC002_VALIDATION_002.json"
BATCH_001 = REPO / "experiments/phase6e/PRODUCTION_GT_BATCH_001.json"

WS_ROOT = REPO / "experiments/phase6e/production-gt-batch-002-packages"
Q_ROOT = REPO / "experiments/phase6e/production-gt-batch-002-queues"
BATCH_OUT = REPO / "experiments/phase6e/PRODUCTION_GT_BATCH_002.json"
SEALED = REPO / "experiments/phase6e/PRODUCTION_GT_BATCH_002_AXIS_MAP_SEALED.json"

ABS_MIN = 20.0
SEED = "PRODUCTION_GT_BATCH_002|ABS_FACE_GE_20|EQUIVARIANT_EDGE_FACE_V1"
BATCH_ID = "PRODUCTION_GT_BATCH_002"
...
def opaque_id(seq: int) -> str:
    return f"MV-PGT2-{_sha(BATCH_ID, f'{seq:04d}')[:12].upper()}"
...
def build() -> dict:
    if any(p.exists() for p in (WS_ROOT, Q_ROOT, BATCH_OUT, SEALED)):
        raise FileExistsError("Refusing to overwrite PRODUCTION_GT_BATCH_002")

    for path in (GATE, PROTOCOL, REOPEN, FREEZE, ACQ, FAILED_001, AS2_002):
        if not path.exists():
    ...
    if reopen.get("ANNOTATION_REOPEN") != "APPROVED":
        raise SystemExit("ANNOTATION_REOPEN not APPROVED")
    freeze = json.loads(FREEZE.read_text(encoding="utf-8"))
    if freeze.get("status") != "FROZEN_BEFORE_BATCH_002_LABELS":
        raise SystemExit("Sampling freeze invalid")
    failed = json.loads(FAILED_001.read_text(encoding="utf-8"))
    if failed.get("status") != "PERMANENT_FAILED_CANDIDATE_ARTIFACT":
        raise SystemExit("Batch 001 failed-candidate artifact missing")

    as2 = json.loads(AS2_002.read_text(encoding="utf-8"))
    validation_sources = set(as2["design"]["sources"])
    batch001_sources = set(failed["sources"])
    if BATCH_001.exists():
        batch001_sources |= set(json.loads(BATCH_001.read_text(encoding="utf-8"))["design"]["sources"])

    acq = json.loads(ACQ.read_text(encoding="utf-8"))
    sources = sorted(acq["selected_ids"])
    n_loc = int(freeze["batch_002_predeclared"]["n_locations"])
    if len(sources) != n_loc:
        raise SystemExit(f"Need {n_loc} sources, have {len(sources)}")

    overlap = sorted((set(sources) & validation_sources) | (set(sources) & batch001_sources))
    if overlap:
        raise SystemExit(f"Source overlap with validation/batch001: {overlap}")

    reviewed = sources_with_human_decisions()
    leaked = [s for s in sources if s in reviewed]
    if leaked:
        raise SystemExit(f"Already reviewed: {leaked}")

    manifest = json.loads(MANIFEST.read_text(encoding="utf-8"))
    records = {r["id"]: r for r in manifest["records"]}
    selection = select_locations(sources, records)

    # Record selected sources into freeze file for audit (sources chosen before labels)
    freeze["batch_002_predeclared"]["selected_sources_materialized"] = sources
    freeze["batch_002_predeclared"]["materialized_at"] = _now()
    freeze["batch_002_predeclared"]["per_location_max_face_steps_pre_label"] = [
        {"source_id": s["source_id"], "center_zyx": s["center_zyx"], "max_face_step": s["max_face_step"], "face_steps": s["face_steps"]}
        for s in selection
    ]
    ...
    for loc_i, loc in enumerate(selection):
        ...
        ws_id = f"MV-EXTERNAL-BOUNDARY-WORKSPACE-{crop_id}-PGT2"
        workspace = {
            ...
            "provenance": {
                ...
                "do_not_train": True,
                "batch_001_failed_candidate_excluded": True,
            ...
            "split": "PRODUCTION_GT_BATCH_002_CANDIDATE",
    ...
    master_questions = []
    for d in ordered:
        crop_id = f"MV-PGT2-{d['location_index'] + 1:02d}"
        ...
        json.dumps(
            {
                ...
                "scientific_boundary": (
                    "PRODUCTION_GT_BATCH_002 candidate under frozen stronger C/E sampling. "
                    "SPEC002+QC unchanged. Not training. Batch 001 permanently failed."
                ),
            },
            indent=2,
        )
    ...
    SEALED.write_text(
        json.dumps(
            {
                "id": "PRODUCTION_GT_BATCH_002_AXIS_MAP_SEALED",
    ...
    batch = {
        ...
        "sampling_freeze_id": "PRODUCTION_GT_BATCH_002_SAMPLING_FREEZE",
        "reopen_decision_id": "AFFINITY_ANNOTATION_REOPEN_DECISION_001",
        "ANNOTATION_REOPEN": "APPROVED",
        "prior_failed_candidate": "experiments/phase6e/PRODUCTION_GT_BATCH_001_FAILED_CANDIDATE.json",
        "cites_validation": {
        ...
        "spec002_unchanged": True,
        "qc_gates_unchanged": True,
        "not_experiment_009": True,
        "do_not_train": True,
        ...
        "design": {
            ...
            "sources": sources,
            "enrichment": f"max single-face |ΔI| >= {ABS_MIN} (AS2-002-justified)",
            "abs_min": ABS_MIN,
            "axis_balanced_ce_crops": True,
            "no_reuse_of_batch_001": True,
            "no_reuse_of_validation_36": True,
            "validation_sources_excluded": sorted(validation_sources),
            "batch_001_sources_excluded": sorted(batch001_sources),
        },
        "blinding": {
        ...
        "launch_example": (
            ...
            r'experiments\phase6e\production-gt-batch-002-queues\MASTER_BATCH.json '
            r'--reviewer matth'
        ),
        "results": None,
        "qc": None,
        ...
        json.dumps(
            {
                ...
                "batch001_overlap": sorted(
                    set(b["design"]["sources"]) & set(b["design"]["batch_001_sources_excluded"])
                ),
            },
            indent=2,
        )

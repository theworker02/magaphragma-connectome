"""Acquire never-reviewed DVID crops near known C/E-magnitude anchors.

Anchors: SURVEY-017/027/035. Label-blind spatial placement only; contrast
screening uses raw intensity after fetch (no human/model labels).
Appends SURVEY-049+ into survey_001 manifest.
"""
...
RECEIPT = REPO / "experiments/phase6e/AFFINITY_SPEC002_VALIDATION_002_SOURCE_ACQUISITION.json"
...
N_FETCH_CAP = 20
MIN_GAP = 64
ABS_AXIS_MIN = 12.0  # prefer crops with ≥1 abs≥12 edge on each axis
SEED = "AFFINITY_SPEC002_VALIDATION_002_SOURCE_ACQUISITION"
# Native XYZ origins of known C/E-magnitude tissue
ANCHORS_XYZ = [
    (1984, 1600, 11456),   # 017
    (10304, 8384, 11456),  # 027
    (11328, 7552, 13440),  # 035
]
...
def candidate_origins(existing: list[dict]) -> list[tuple[int, int, int]]:
    # Offsets in voxels around each anchor (64-aligned ring)
    rings = [192, 256, 320, 384, 448, 512, 640, 768]
    dirs = []
    for dx in (-1, 0, 1):
        for dy in (-1, 0, 1):
            for dz in (-1, 0, 1):
...
def axis_high_counts(arr: np.ndarray) -> dict[str, int]:
    m = 4
    z, y, x = arr.shape
    out = {}
...
def main() -> int:
    ...
    assert next_ord == 49, next_ord

    origins = candidate_origins(existing)
    if len(origins) < N_TARGET:
        raise SystemExit(f"Only {len(origins)} candidate origins")
    ...
    for i, origin in enumerate(origins):
        ...
        if len(payload) != expected:
            raise RuntimeError(f"{identifier}: bad size {len(payload)}")
        ...
        balanced = all(counts[a] >= 8 for a in ("Z", "Y", "X"))
        rec = {
            "id": identifier,
            ...
            "acquisition": {
                "batch_id": "AFFINITY_SPEC002_VALIDATION_002_SOURCE_ACQUISITION",
                "purpose": "SPEC002_DUAL_CLASS_ABS_ENRICHED_VALIDATION",
                "near_ce_anchors": True,
    ...
    # Prefer axis-balanced C/E-scale crops; take top N_TARGET by min(Z,Y,X) high-count
    ranked = sorted(
        fetched,
        key=lambda r: (
            0 if r["acquisition"]["axis_balanced_ce_scale"] else 1,
            -min(r["acquisition"]["axis_abs_ge12_counts"][a] for a in ("Z", "Y", "X")),
            -r["intensity"]["std"],
        ),
    )
    keep = ranked[:N_TARGET]
    keep_ids = {r["id"] for r in keep}
    # Still append ALL fetched to manifest (extras are reserve never-reviewed)
    manifest["records"].extend(fetched)
    manifest["extended_at"] = _now()
    manifest["extension_002"] = {
        "id": "AFFINITY_SPEC002_VALIDATION_002_SOURCE_ACQUISITION",
        "n_added": len(fetched),
        "ids": [r["id"] for r in fetched],
        "selected_for_validation_002": sorted(keep_ids),
    }
    ...
    receipt = {
        ...
        "n_selected": len(keep),
        "abs_axis_min": ABS_AXIS_MIN,
        "anchors_xyz": [list(a) for a in ANCHORS_XYZ],
        ...
        "prohibited_inputs": ["human_labels", "g3_labels", "model_predictions", "segmentation"],
    }
    RECEIPT.write_text(json.dumps(receipt, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    print(json.dumps({"fetched": len(fetched), "selected": sorted(keep_ids), "receipt": str(RECEIPT)}, indent=2))
    return 0
